...
class RemoteMessageStrategy(pyctor.types.MessageStrategy[pyctor.types.T]):
    ...
    def transform_send_message(self, me: pyctor.types.Ref[pyctor.types.T], msg: pyctor.types.T) -> Any:
        ...
        """
        return pyctor.multiprocess.messages.MessageCommand(
            ref=me, type=".".join([type(msg).__module__, type(msg).__name__]), msg=msgspec.Raw(pyctor.configuration._default_encoder.encode(msg))
        )
